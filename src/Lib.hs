...
emptyWorld :: World
emptyWorld = World (Typing []) M.empty [] 200 1
...
renderWorld w = pictures $
    renderOrbitals (worldViewScale w) (worldValScale w) (worldAtoms w) (worldOrbitals w) :
...
handleEvent event w = case inputState w of
    ...
    (Editing s) -> case event of
        ...
        (EventKey (Char 'f') Down _ _) -> w{worldOrbitals = testOrbs atoms}
        (EventKey (Char '=') Down _ _) -> w{worldOrbitals = drop 40 orbs}
        (EventKey (Char '[') Down _ _) -> traceShow (head orbs) $ w{worldOrbitals = tail orbs ++ [head orbs]}
        (EventKey (Char ']') Down _ _) -> w{worldOrbitals = last orbs : init orbs}
        (EventKey (SpecialKey KeyDelete) Down _ _) -> w{worldOrbitals = [], worldAtoms = M.delete s atoms}
        _ -> both s
    where atoms = worldAtoms w
          ...
          both s = case event of
              (EventKey (MouseButton LeftButton) Down _ (x,y)) -> w{worldAtoms = M.insert s (emptyAtom [num $ x/viewScale, num $ y/viewScale, 0]) atoms}
              (EventKey (MouseButton WheelDown) Down _ _) -> w{worldViewScale = viewScale / 2, worldValScale = valScale * (2 ** (d/2))}
              (EventKey (MouseButton WheelUp  ) Down _ _) -> w{worldViewScale = viewScale * 2, worldValScale = valScale / (2 ** (d/2))} 
              _ -> w
