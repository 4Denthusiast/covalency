...
inverseIterants :: (InnerProduct Rl a, Ord a) => Matrix a -> Rl -> Linear Rl a -> [Linear Rl a]
...
eigenvecNear :: (InnerProduct Rl a, Ord a) => Matrix a -> Rl -> Linear Rl a
...
negativeEigenvecs :: (InnerProduct Rl a, Ord a) => Matrix a -> [(Rl,Linear Rl a)]
...
negativeEigenvecsFrom :: (InnerProduct Rl a, Ord a) => Matrix a -> Rl -> [(Rl,Linear Rl a)]
...
eigenvalsFrom :: (InnerProduct Rl a, Ord a) => Matrix a -> Rl -> [Rl]
...
removeEigenval :: (InnerProduct Rl a, Ord a) => Rl -> Matrix a -> Matrix a
removeEigenval μ m = flip offsetMat (-μ) $ uncurry (traceShow . (μ,) . (length)) $ removeKernel $ offsetMat m μ

removeKernel :: forall a. (InnerProduct Rl a, Ord a) => Matrix a -> ([Linear Rl a], Matrix a)
removeKernel m0 = removeKernel' m0 (idMat xs0) xs0 []
    where xs0 = M.keys m0
          ...
          removeKernel' m m' (x:xs) ker = {-trace ("m:\n"++showMatrix m++"\nm':\n"++showMatrix m'++"\nm' * m:\n"++showMatrix (matTimes m' <$> m)++"\n") $-} let
              ...
              in
                  if abs a > 1e-6* abs a' then succeed else fail
...
eigenvalNear :: (Ord a, InnerProduct Rl a) => Matrix a -> Rl -> Rl
